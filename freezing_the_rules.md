...
- `1/alpha_s(mZ)` (legacy benchmark; see below)
- `1/alpha_s_1loop(Q)`

Strong strict target (frozen):
...
- `alpha2(alpha(mZ),sin2_on_shell)` (derived using the on-shell mixing angle; strict weak target)
...
- Under the **frozen inverse** convention (strict targets):
...
- `1/alpha_s(mZ)`:
  - C=60, m=4 (≈ +3.21%)
- `1/alpha_w(mZ)` (proxy from $g\approx 0.652$):
  - C=120, m=3 (≈ -4.17%)
